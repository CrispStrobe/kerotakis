...
- [ ] **Lane assignment**: `runtime` (ships in app), `build_oracle` (dev only),
      `external_oracle` (never fetched in CI), or `candidate-data` (a pinned
      import snapshot sitting in quarantine, cleared for nothing yet — a
      BRD-003 adapter's output lives here until a per-field review moves it).
      A candidate lane is not a runtime lane: putting a licence there is not a
      claim that it may ship.
