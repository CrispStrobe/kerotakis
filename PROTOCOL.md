...
|---|---|---|
| `hello` | done | `{}` → `{ protocol, can_solve, engine_loaded, load_failure, aqueous_note, engine_version, git_rev, registers, narration }`. Additive 2026-09-22 (GUI-127): `narration` lists the narration switches this engine answers — `["routing"]` today — so a shell can offer the control only where it takes effect rather than sending a command into a `catch`. Absent from an older host, which means no switches. `git_rev` is stamped by the build (`KEROTAKIS_GIT_REV`; null in unstamped dev builds). `packs` carries the WEB-003 inventory (kerotakis_core::packs_manifest; empty content_hash = built in, not yet independently deliverable — the honest pre-pipeline state); `load_pack` itself remains the open half. Must be answerable before any pack loads. |
| `step` | done | `{ operator_json }` → `{ events, rendered, charts, ionic, scene, bench }` — `events` is the serde `Vec<Event>`, `rendered` the prose at the current register, `charts` the CAP-3 `Chart[]` the step's events earned (empty when none; first producer: the titration curve), `ionic` the GUI-092 `NetIonic[]` derived from the solved speciation (empty when none, which is the common case), `scene` the render model (one round trip repaints the bench). Additive 2026-09-07 (GUI-052): `routes` is the step's routing evidence, `[{ solver, kind, chemistry, outcome, vessel?, reason? }]` — which solver was asked, in order, and what it answered. `kind` is `computed` / `curated` / `qualitative`; `outcome` is `"not_applicable"`, `"failed"`, or `{ succeeded: { event_count } }`; `reason` is the solver's OWN sentence for declining, present only where the solver overrides `capability()` with one worth reading; `vessel` names which beaker that pass examined, because a step may equilibrate more than one. Empty — not absent — for an operator that equilibrates nothing (`new`), which is a claim: this step routed through no solver, rather than inheriting the last step's. Absent from an older host. Additive 2026-09-17 (#642): an event's `provenance` may carry `routing_phrase` beside `routing` — the same sentence as a `Phrase` recipe (`{ key, en, slots }`), so a host that composes its own prose can render it in the reader's language. `routing` remains the sentence in English and is what every existing consumer reads; a host that does not know about the recipe is unaffected, and the field is omitted entirely where a solver has not composed one. Additive 2026-09-18 (#653): `solution_routed` is a second event carrying a `provenance` — the aqueous one, which dataset answers this beaker and why. It fires **on change**, not on every characterisation: the aqueous solver runs far more often than a reader wants a line (five times over three commands of `aq-023`), so the engine speaks only when the engine, the dataset, the model or the SHAPE of the routing recipe differs from what this vessel was last announced as. A step that solved and says nothing is the routing STANDING, not a routing missing; a host must not read the absence as a gap. Numbers inside a reason are not part of the comparison — the concentrated-brine route carries the molality in its own sentence and that moves with every spoonful. Additive 2026-09-18 (#655): a `provenance` may also carry `dataset_phrase` and `model_phrase` beside `dataset` and `model`, the same recipe shape (`{ key, en, slots }`). Both fields were a NAME with an English sentence welded to it — *wateq4f.dat plus USBM IC 9429 reference-temperature complexes, with the reviewed Sander HBr gas-uptake slice* — and the recipe carries the file name in a text slot the catalogue never translates, with the clauses around it as the part it does. `dataset` and `model` remain the sentence in English and are what every existing consumer reads; each is the recipe rendered in the source language, so they are byte-identical to what they were. Both are omitted where the claim is a name and nothing else (`NASA CEA thermo.inp`), because there is no sentence to translate. A host that does not know about them is unaffected. Changed 2026-09-18 (owner's ruling): `solution_routed` now fires from **three** aqueous paths, not one — the direct solve, MIX, and solvent-only characterisation. The last two wrote a `Provenance` into the vessel's state that no event carried, so a beaker filled by combining two others, or one holding only solvent, held provenance no reader could reach. The fire-on-change rule is unchanged and is now shared by all three: a vessel is told when the engine, the dataset, the model or the SHAPE of the routing recipe differs from what it was last announced as, so a beaker of water says once which relation answered it and then says nothing while that holds. One consequence for a host reading `routes`: a route's `event_count` counts ANSWERS and a routing announcement is not one, so a solver whose only event was `solution_routed` still reports `{ succeeded: { event_count: 0 } }` — otherwise a beaker of plain water would read as a solver that computed something. |
...
| `set_announce_routing` | done (GUI-127, 2026-09-22) | `{ on }` → `{}`. Whether `rendered` carries the `solution_routed` announcement. Presentation only; never re-solves, and **never changes `events` or `routes`** — the event and its provenance still arrive in full, so the provenance drawer answers the same either way. A SECOND axis beside the register and deliberately not folded into it: the announcement is a paragraph at lv3 (engine · dataset · activity model · the clause explaining the choice), and on a German bench it was 454 of one step's 550 rendered characters, so the only way to be rid of it was to leave lv3 and give up every number lv3 was turned on for. `true` is the default and is what a host that is never asked renders, so nothing changes for a shell that does not know about it. The engine decides, not the shell: `Narration` filters the EVENT, where "the line that starts with Route" would be a fact about one language's rendering at one register. Refused by name on an older host; `hello.narration` (`["routing"]`) is how a shell knows the switch is answered before it offers it. |
