...
- **Remaining acceptance items (not met as counted):**
  ...
  3. **The "≥25 prompts graduate through a reviewed reduced mechanism" floor is
     ...
     mechanism.
- **Landed 2026-09-07 (#510):** the portable Cantera-YAML parser accepts the
  **reversible** three-body and falloff forms — BRD-040 §4 items 1 and 2, and
  the one refusal standing between the shipped subset and `gri30.yaml` /
  `air.yaml`. The exclusion of *falloff* in the line below is therefore now a
  statement about the shipped mechanism packs, not about the parser.
