...
An engine catalog still loading is shown as unknown rather than prematurely
ready, and instrument names are resolved through the existing equipment model.
...
| K14 | Naked egg | membrane water transfer | computed + visible | `naked_egg` keeps its inventory behind a bounded semipermeable membrane; water moves with the osmotic gradient, and the existing experiment/mission bench now keeps the signed cumulative mol/g transfer visible and accessible. Shell removal remains the separate chalk/vinegar surrogate, with no yolk, elasticity, object-size or final-equilibrium claim |
