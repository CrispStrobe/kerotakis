...
> **Status note, 2026-09-08.** This contract is still valid and still frozen,
> but its preconditions changed: PR #504 is closed, so "run only after PR #504
> CI passes" now means run against a `kero` built from current `main`. The
> checks below must not be edited to match whatever that run produces.
