...
Covers the first fleet, cases 1-50. The run directories this originally read
(`results/`, `followups/`, `followups-50/`) are no longer in the repository:
they are in the evidence archive named in `README.md`. Both arguments are
therefore explicit, so the tool can never silently point at a path that is
gone. Recreate the inputs with `run.py --out <fresh directory>`, or unpack the
archive and pass the extracted directories.
"""
import argparse
import json
import math
...
def main():
    ...
    parser.add_argument("directories", nargs="+", type=pathlib.Path,
                        help="run directories written by run.py, each holding summary.json")
    parser.add_argument("--out", required=True, type=pathlib.Path,
                        help="new report file; the historical report is in the evidence archive")
