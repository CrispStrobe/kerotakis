 ...
 *
 * Both are needed by two components that are not related: the strip
...
class InstrumentSurface {
  ...
  recent = $state<string[]>([]);
  /**
   * GUI-111 retired `sets`, and with it `showSets()` and the stored
   * `kerotakis.equipment.sets` key. It said whether the cupboard named its
   * tools after the activity kits; it is not a choice any more, because
   * the kit names moved onto the tools themselves — into the search
   * haystack and into the (i) panel — where they need no switch.
   */
  #hydrated = false;
