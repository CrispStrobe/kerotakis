...
export interface ComparisonCell { sample: CodexAssertionSample; value: number | null; unit: string }
...
export function metricValue(scene: Scene | null, sample: CodexAssertionSample, vesselOffset = 0): { value: number | null; unit: string } {
  const all = scene?.vessels ?? [];
  const number = vesselNumber(sample.vessel);
  const vessels = number === undefined ? all : all.filter((v) => v.id === Number(number) - 1 + vesselOffset);
...
export function comparisonRows(assertions: readonly CodexAssertion[], snapshots: readonly ComparisonSnapshot[], vesselOffset = 0): ComparisonRow[] {
  return assertions.map((assertion) => {
    const cells = assertion.samples.map((sample) => ({ sample, ...metricValue(snapshots.find((s) => s.step === sample.step)?.scene ?? null, sample, vesselOffset) }));
