...
pub const SUBLIMATION_ENTHALPIES: &[LatentHeat] = &[LatentHeat {
    ...
    kj_per_mol: 25.2,
    provenance: "Enthalpy of sublimation of carbon dioxide at its 194.7 K normal sublimation point, 25.2 kJ/mol. NO SOURCE IS CLAIMED FOR THIS NUMBER. It read \"as commonly tabulated from NIST/CODATA-class evaluated data\" until 2026-09-13, and that phrase is withdrawn because it is not a citation: it names no document a reader could open, and the half of it that named anything named NIST SRD material, which PLAN.md's audit marks copyrighted and not to be redistributed. NASA CEA cannot replace it - vendor/nasa-cea/thermo.inp carries gaseous CO2 but no solid record, so there is no condensed pair to difference. What stands behind the value is the arithmetic below and nothing else. Sanity check a reviewer can run without a book: it is the sum of the tabulated 8.65 kJ/mol enthalpy of fusion at the triple point and about 16.7 kJ/mol of vaporisation there, and it is the number that makes 5 g of dry ice cool 100 g of water by about 6.8 K, which is what a kitchen thermometer reads",
}];
...
///
/// The organic rows are a different problem, and it is not solved. CEA has
/// no condensed methanol, acetone, propan-2-ol, hexane, ethyl acetate,
/// acetic acid or naphthalene, so nothing on disk reaches them. Their
/// handbook citations are **withdrawn** rather than replaced — along with
/// the "agrees with NIST SRD 69" line each of them carried, which was a
/// claim and not a citation — and each row now says in its own provenance
/// that nothing supports it.
///
/// ## Why the unsupported rows are not simply deleted
///
/// Because deleting them would be dishonest in the other direction, and
/// this is worth spelling out because "the claim stops" reads like "the
/// row goes".
///
/// [`PhaseRoute::vaporising`] and [`PhaseRoute::condensing`] reach this
/// table through a `filter_map` with `?`. A solid whose melting point is
...
/// melting point and calling it solid. That is precisely the P3s
...
/// the registry's transition temperature with it, so that `melts_at`
/// returns `None` and the bench has no transition to be silently wrong
/// about — and that is a change to the registry tranche, not to this
/// table. Until that is done the numbers stay, unsupported and saying so.
pub const FUSION_ENTHALPIES: &[LatentHeat] = &[
    LatentHeat {
        species: "ethanol",
        // 4.93 kJ/mol at the 159.01 K melting point already in the registry.
        kj_per_mol: 4.93,
        provenance: "Ethanol enthalpy of fusion 4.93 kJ/mol at its normal melting point. NO SOURCE IS CLAIMED FOR THIS NUMBER. It cited the NIST Chemistry WebBook, SRD 69 until 2026-09-13; PLAN.md's provenance audit marks NIST SRD material copyrighted and not to be scraped or redistributed, so a WebBook URL cannot stand behind a shipped value and the citation is withdrawn rather than reworded. The value is roughly a fifth of water's 6.01 kJ/mol per mole and about a ninth per gram, which is why a small pour of liquid nitrogen can freeze ethanol but would barely dent the same mass of water",
    },
    LatentHeat {
        species: "methanol",
        // 3.18 kJ/mol at the 175.62 K melting point in the registry.
        kj_per_mol: 3.18,
        provenance: "Methanol enthalpy of fusion 3.18 kJ/mol at its normal melting point. NO SOURCE IS CLAIMED FOR THIS NUMBER. It cited the CRC Handbook of Chemistry and Physics, 97th edition, until 2026-09-13, with a note that the printed edition had not been opened and that the value agreed with NIST Chemistry WebBook SRD 69. Both halves of that are withdrawn: PLAN.md's provenance audit puts the handbook on the commercial row and the WebBook on the do-not-redistribute row, and \"agrees with SRD 69\" was a claim rather than a citation. What is left is commentary: this is the value the general references carry for this substance, and it is supported by nothing stronger than that. The number is NOT removed, and the reason is a correctness one rather than caution - `vaporising` and `condensing` reach this table through a filter_map, so a row that disappears does not make the bench refuse the transition, it makes the bench hold a solid above its melting point and call it solid, which is the P3s bug this whole table was added to fix. An honest shrink here has to take the registry's transition temperature with it, and that is a larger decision than withdrawing a citation. To restore support this row needs its own primary measurement with a DOI, in the shape `literature/hartley-campbell-iodine-water` uses in the registry; vendor/nasa-cea/thermo.inp cannot help, because CEA carries no condensed record for this substance.",
    },
    LatentHeat {
        species: "propanone",
        // 5.69 kJ/mol at the 178.45 K melting point in the registry.
        kj_per_mol: 5.69,
        provenance: "Propanone (acetone) enthalpy of fusion 5.69 kJ/mol at its normal melting point. NO SOURCE IS CLAIMED FOR THIS NUMBER. It cited the CRC Handbook of Chemistry and Physics, 97th edition, until 2026-09-13, with a note that the printed edition had not been opened and that the value agreed with NIST Chemistry WebBook SRD 69. Both halves of that are withdrawn: PLAN.md's provenance audit puts the handbook on the commercial row and the WebBook on the do-not-redistribute row, and \"agrees with SRD 69\" was a claim rather than a citation. What is left is commentary: this is the value the general references carry for this substance, and it is supported by nothing stronger than that. The number is NOT removed, and the reason is a correctness one rather than caution - `vaporising` and `condensing` reach this table through a filter_map, so a row that disappears does not make the bench refuse the transition, it makes the bench hold a solid above its melting point and call it solid, which is the P3s bug this whole table was added to fix. An honest shrink here has to take the registry's transition temperature with it, and that is a larger decision than withdrawing a citation. To restore support this row needs its own primary measurement with a DOI, in the shape `literature/hartley-campbell-iodine-water` uses in the registry; vendor/nasa-cea/thermo.inp cannot help, because CEA carries no condensed record for this substance.",
    },
    LatentHeat {
        species: "isopropanol",
        // 5.37 kJ/mol at the 185.25 K melting point in the registry.
        kj_per_mol: 5.37,
        provenance: "Propan-2-ol enthalpy of fusion 5.37 kJ/mol at its normal melting point. NO SOURCE IS CLAIMED FOR THIS NUMBER. It cited the CRC Handbook of Chemistry and Physics, 97th edition, until 2026-09-13, with a note that the printed edition had not been opened and that the value agreed with NIST Chemistry WebBook SRD 69. Both halves of that are withdrawn: PLAN.md's provenance audit puts the handbook on the commercial row and the WebBook on the do-not-redistribute row, and \"agrees with SRD 69\" was a claim rather than a citation. What is left is commentary: this is the value the general references carry for this substance, and it is supported by nothing stronger than that. The number is NOT removed, and the reason is a correctness one rather than caution - `vaporising` and `condensing` reach this table through a filter_map, so a row that disappears does not make the bench refuse the transition, it makes the bench hold a solid above its melting point and call it solid, which is the P3s bug this whole table was added to fix. An honest shrink here has to take the registry's transition temperature with it, and that is a larger decision than withdrawing a citation. To restore support this row needs its own primary measurement with a DOI, in the shape `literature/hartley-campbell-iodine-water` uses in the registry; vendor/nasa-cea/thermo.inp cannot help, because CEA carries no condensed record for this substance.",
    },
    LatentHeat {
        species: "hexane",
        // 13.08 kJ/mol at the 177.88 K melting point in the registry.
        kj_per_mol: 13.08,
        provenance: "Hexane enthalpy of fusion 13.08 kJ/mol at its normal melting point. NO SOURCE IS CLAIMED FOR THIS NUMBER. It cited the CRC Handbook of Chemistry and Physics, 97th edition, until 2026-09-13, with a note that the printed edition had not been opened and that the value agreed with NIST Chemistry WebBook SRD 69. Both halves of that are withdrawn: PLAN.md's provenance audit puts the handbook on the commercial row and the WebBook on the do-not-redistribute row, and \"agrees with SRD 69\" was a claim rather than a citation. What is left is commentary: this is the value the general references carry for this substance, and it is supported by nothing stronger than that. The number is NOT removed, and the reason is a correctness one rather than caution - `vaporising` and `condensing` reach this table through a filter_map, so a row that disappears does not make the bench refuse the transition, it makes the bench hold a solid above its melting point and call it solid, which is the P3s bug this whole table was added to fix. An honest shrink here has to take the registry's transition temperature with it, and that is a larger decision than withdrawing a citation. To restore support this row needs its own primary measurement with a DOI, in the shape `literature/hartley-campbell-iodine-water` uses in the registry; vendor/nasa-cea/thermo.inp cannot help, because CEA carries no condensed record for this substance.",
    },
    LatentHeat {
        species: "ethyl_acetate",
        // 10.48 kJ/mol at the 189.55 K melting point in the registry.
        kj_per_mol: 10.48,
        provenance: "Ethyl acetate enthalpy of fusion 10.48 kJ/mol at its normal melting point. NO SOURCE IS CLAIMED FOR THIS NUMBER. It cited the CRC Handbook of Chemistry and Physics, 97th edition, until 2026-09-13, with a note that the printed edition had not been opened and that the value agreed with NIST Chemistry WebBook SRD 69. Both halves of that are withdrawn: PLAN.md's provenance audit puts the handbook on the commercial row and the WebBook on the do-not-redistribute row, and \"agrees with SRD 69\" was a claim rather than a citation. What is left is commentary: this is the value the general references carry for this substance, and it is supported by nothing stronger than that. The number is NOT removed, and the reason is a correctness one rather than caution - `vaporising` and `condensing` reach this table through a filter_map, so a row that disappears does not make the bench refuse the transition, it makes the bench hold a solid above its melting point and call it solid, which is the P3s bug this whole table was added to fix. An honest shrink here has to take the registry's transition temperature with it, and that is a larger decision than withdrawing a citation. To restore support this row needs its own primary measurement with a DOI, in the shape `literature/hartley-campbell-iodine-water` uses in the registry; vendor/nasa-cea/thermo.inp cannot help, because CEA carries no condensed record for this substance.",
    },
    LatentHeat {
        species: "CH3COOH",
        // 11.73 kJ/mol at the 289.75 K melting point in the registry.
        kj_per_mol: 11.73,
        provenance: "Acetic acid enthalpy of fusion 11.73 kJ/mol at its 289.75 K melting point. NO SOURCE IS CLAIMED FOR THIS NUMBER. It cited the CRC Handbook of Chemistry and Physics, 97th edition, until 2026-09-13, with a note that the printed edition had not been opened and that the value agreed with NIST Chemistry WebBook SRD 69. Both halves of that are withdrawn: PLAN.md's provenance audit puts the handbook on the commercial row and the WebBook on the do-not-redistribute row, and \"agrees with SRD 69\" was a claim rather than a citation. What is left is commentary: this is the value the general references carry for this substance, and it is supported by nothing stronger than that. The number is NOT removed, and the reason is a correctness one rather than caution - `vaporising` and `condensing` reach this table through a filter_map, so a row that disappears does not make the bench refuse the transition, it makes the bench hold a solid above its melting point and call it solid, which is the P3s bug this whole table was added to fix. An honest shrink here has to take the registry's transition temperature with it, and that is a larger decision than withdrawing a citation. To restore support this row needs its own primary measurement with a DOI, in the shape `literature/hartley-campbell-iodine-water` uses in the registry; vendor/nasa-cea/thermo.inp cannot help, because CEA carries no condensed record for this substance. This is the row that makes \"glacial\" mean something: pure acetic acid at 16.6 degrees Celsius is a solid, and a cold laboratory really does freeze the bottle. It says nothing about vinegar, which is acetic acid dissolved in water and freezes on the solvent's depressed point instead",
    },
    LatentHeat {
        species: "naphthalene",
        // 19.01 kJ/mol at the 353.4 K melting point in the registry.
        kj_per_mol: 19.01,
        provenance: "Naphthalene enthalpy of fusion 19.01 kJ/mol at its 353.4 K melting point. NO SOURCE IS CLAIMED FOR THIS NUMBER. It cited the CRC Handbook of Chemistry and Physics, 97th edition, until 2026-09-13, with a note that the printed edition had not been opened and that the value agreed with NIST Chemistry WebBook SRD 69. Both halves of that are withdrawn: PLAN.md's provenance audit puts the handbook on the commercial row and the WebBook on the do-not-redistribute row, and \"agrees with SRD 69\" was a claim rather than a citation. What is left is commentary: this is the value the general references carry for this substance, and it is supported by nothing stronger than that. The number is NOT removed, and the reason is a correctness one rather than caution - `vaporising` and `condensing` reach this table through a filter_map, so a row that disappears does not make the bench refuse the transition, it makes the bench hold a solid above its melting point and call it solid, which is the P3s bug this whole table was added to fix. An honest shrink here has to take the registry's transition temperature with it, and that is a larger decision than withdrawing a citation. To restore support this row needs its own primary measurement with a DOI, in the shape `literature/hartley-campbell-iodine-water` uses in the registry; vendor/nasa-cea/thermo.inp cannot help, because CEA carries no condensed record for this substance. Naphthalene is the substance school cooling-curve experiments are actually run on, because the plateau sits in a water bath's reach. No enthalpy of vaporisation is claimed for it here: its 491 K boiling point is outside what that experiment goes near, and a row would install a boil this tranche has not checked",
    },
    LatentHeat {
        species: "Pb",
...
pub const VAPORISATION_ENTHALPIES: &[LatentHeat] = &[
    LatentHeat {
        ...
        // 5.58 kJ/mol at the 77.364 K normal boiling point, NBS TN 129 Table 2.
        kj_per_mol: 5.58,
        provenance: "Nitrogen enthalpy of vaporisation 5.58 kJ/mol at its 77.364 K normal boiling point. T. R. Strobridge, The Thermodynamic Properties of Nitrogen from 64 to 300 K between 0.1 and 200 Atmospheres, NBS Technical Note 129, National Bureau of Standards Cryogenic Engineering Laboratory, January 1962, doi:10.6028/NBS.TN.129, Table 2 (Thermodynamic Properties of Nitrogen at Saturation): the 1.000 atm row gives delta-H(vap) = 199.260 J/g, which at that document's own stated basis of M = 28.016 g/mol is 5582 J/mol, and 5582 J/mol with the modern 28.0134 as well. Retrieved 2026-09-13 from https://nvlpubs.nist.gov/nistpubs/Legacy/TN/nbstechnicalnote129.pdf. NBS Technical Notes are works of United States Government employees and are not subject to copyright in the United States; this document carries no copyright notice of any kind. Kerotakis transcribes one factual measurement and redistributes none of the source text or tables, the footing `literature/hartley-campbell-iodine-water` already stands on in the registry. THIS ROW MOVED, 5.6 to 5.58. The retired figure cited the NIST Chemistry WebBook SRD 69, which PLAN.md's audit marks copyrighted and not to be redistributed, and it was quoted to two significant figures because that is what the WebBook displayed; TN 129 carries six. NASA CEA was the other candidate and is wrong here for a reason worth keeping: its N2(L) record is an assigned enthalpy with no polynomial, and pairing it with CEA's IDEAL-gas N2 record gives 5.68 kJ/mol. TN 129's is a real-gas saturated-vapour enthalpy, so the 1.7 per cent between them IS the vapour's non-ideality at 77 K - the same overshoot CEA shows for water, ethanol and methanol, and the reason no vaporisation row on this bench cites CEA. It is about a fourteenth of water's 40.65 kJ/mol per mole, which is why liquid nitrogen boils away rapidly",
    },
    LatentHeat {
        species: "ethanol",
        // 38.56 kJ/mol at the 351.39 K boiling point in the registry.
        kj_per_mol: 38.56,
        provenance: "Ethanol enthalpy of vaporisation 38.56 kJ/mol at its normal boiling point. NO SOURCE IS CLAIMED FOR THIS NUMBER. It cited the NIST Chemistry WebBook, SRD 69 until 2026-09-13, and PLAN.md's audit marks NIST SRD material copyrighted and not to be redistributed. NASA CEA was checked and rejected for the same reason as nitrogen: vendor/nasa-cea/thermo.inp does carry C2H5OH(L) over 159-390 K, which spans this boiling point, but H(gas, Tb) - H(liquid, Tb) comes to 39.64 kJ/mol because the gas record is ideal. That 2.8 per cent is the vapour's non-ideality, which is exactly what an enthalpy of vaporisation at one atmosphere is not allowed to ignore. CEA is the right source for a fusion enthalpy, where both phases are condensed and the ideal-gas reference cancels; it is the wrong source for a boil. Against water's 40.65 kJ/mol it is nearly the same per mole and less than half per gram, which is why a spirit burner empties so much faster than a kettle",
    },
    LatentHeat {
        species: "methanol",
        // 35.21 kJ/mol at the 337.85 K boiling point in the registry.
        kj_per_mol: 35.21,
        provenance: "Methanol enthalpy of vaporisation 35.21 kJ/mol at its normal boiling point. NO SOURCE IS CLAIMED FOR THIS NUMBER. It cited the CRC Handbook of Chemistry and Physics, 97th edition, until 2026-09-13, with a note that the printed edition had not been opened and that the value agreed with NIST Chemistry WebBook SRD 69. Both halves of that are withdrawn: PLAN.md's provenance audit puts the handbook on the commercial row and the WebBook on the do-not-redistribute row, and \"agrees with SRD 69\" was a claim rather than a citation. What is left is commentary: this is the value the general references carry for this substance, and it is supported by nothing stronger than that. The number is NOT removed, and the reason is a correctness one rather than caution - `vaporising` and `condensing` reach this table through a filter_map, so a row that disappears does not make the bench refuse the transition, it makes the bench hold a solid above its melting point and call it solid, which is the P3s bug this whole table was added to fix. An honest shrink here has to take the registry's transition temperature with it, and that is a larger decision than withdrawing a citation. To restore support this row needs its own primary measurement with a DOI, in the shape `literature/hartley-campbell-iodine-water` uses in the registry; vendor/nasa-cea/thermo.inp cannot help, because CEA carries no condensed record for this substance.",
    },
    LatentHeat {
        species: "propanone",
