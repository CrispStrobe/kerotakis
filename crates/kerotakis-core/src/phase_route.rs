...
pub const VAPORISATION_ENTHALPIES: &[LatentHeat] = &[
    LatentHeat {
        ...
        kj_per_mol: 5.6,
        provenance: "Nitrogen enthalpy of vaporisation 5.6 kJ/mol at 77 K. NO SOURCE IS CLAIMED FOR THIS NUMBER. It cited the NIST Chemistry WebBook, SRD 69 until 2026-09-13, and PLAN.md's audit marks NIST SRD material copyrighted and not to be redistributed. NASA CEA was checked as the replacement and does not carry it cleanly: vendor/nasa-cea/thermo.inp has an N2(L) reactant record at 77.352 K, but it is an assigned enthalpy with no polynomial, and pairing it with CEA's IDEAL-gas N2 record gives about 5.68 kJ/mol - the ideal-gas latent heat, not the real one, and the two per cent gap IS the vapour's non-ideality at 77 K. Adopting it would be trading a licence problem for a physics one. It is about a fourteenth of water's 40.65 kJ/mol per mole, which is why liquid nitrogen boils away rapidly",
    },
    LatentHeat {
        species: "ethanol",
