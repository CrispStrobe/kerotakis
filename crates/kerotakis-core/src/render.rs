...
pub fn render_event_in(event: &Event, register: Register, locale: Locale) -> String {
    match event {
        ...
        Event::ChemiluminescenceObserved { vessel, relative_intensity, half_life_s, elapsed_s, temperature, oxidant_moles } => match register.level() {
            1 => locale.fill("event.chemiluminescence.lv1", "The mixture in {vessel} glows blue. Warmer mixtures shine more brightly now, but fade sooner.", &[("vessel", &vessel.to_string())]),
            2 => locale.fill("event.chemiluminescence.lv2", "{vessel}: relative blue-light intensity {intensity}; estimated half-life {half}s at {temp} K", &[("vessel", &vessel.to_string()), ("intensity", &locale.number(format!("{relative_intensity:.2}"))), ("half", &locale.number(format!("{half_life_s:.1}"))), ("temp", &locale.number(format!("{:.1}", temperature.0)))]),
            _ => locale.fill("event.chemiluminescence.lv3", "{vessel}: bounded luminol-system relative intensity {intensity} after {elapsed} s at {temp} K with {oxidant} mol H2O2; Arrhenius-shaped teaching response, not photon yield or product speciation, and not the peroxyoxalate chemistry of a commercial glow stick", &[("vessel", &vessel.to_string()), ("intensity", &locale.number(format!("{relative_intensity:.4}"))), ("elapsed", &locale.number(format!("{elapsed_s:.1}"))), ("temp", &locale.number(format!("{:.2}", temperature.0))), ("oxidant", &locale.number(format!("{:.6}", oxidant_moles.0)))]),
        },
        ...
        Event::Partitioned { vessel, species, fraction_lower } => match register.level() {
            1 => locale.fill(
                ...
                &[("species", species::lookup(species).map(|d| d.name).unwrap_or(species.0.as_str())), ("vessel", &vessel.to_string())],
            ),
            2 => locale.fill(
                "event.partitioned.lv2",
                "{vessel}: {species} split between the layers — {fraction_lower}% in the lower, the rest dissolved in the upper",
                &[("vessel", &vessel.to_string()), ("species", species::lookup(species).map(|d| d.name).unwrap_or(species.0.as_str())), ("fraction_lower", &locale.number(format!("{:.0}", fraction_lower * 100.0)))],
            ),
            _ => locale.fill(
        ...
        Event::Drained { from, to, solvent, moles } => match register.level() {
            ...
            2 => locale.fill(
                ...
                &[("from", &from.to_string()), ("to", &to.to_string()), ("moles", &locale.number(format!("{:.3}", moles.0))), ("species", species::lookup(solvent).map(|d| d.name).unwrap_or(solvent.0.as_str()))],
            ),
            _ => locale.fill(
                "event.drained.lv3",
        ...
        Event::LayersFormed { vessel, upper, lower } => match register.level() {
            ...
            2 => locale.fill(
                ...
                &[("vessel", &vessel.to_string()), ("species", species::lookup(upper).map(|d| d.name).unwrap_or(upper.0.as_str())), ("species2", species::lookup(lower).map(|d| d.name).unwrap_or(lower.0.as_str()))],
            ),
            _ => locale.fill(
