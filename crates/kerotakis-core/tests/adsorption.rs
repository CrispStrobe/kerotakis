...
fn the_carbon_takes_most_of_the_dye_and_not_all_of_it() {
    ...
    assert!(held(&v) > 0.0, "something was actually adsorbed");
...
fn no_carbon_means_no_adsorption() {
    ...
    let events = adsorption::equilibrate(&mut v);
    assert!(events.is_empty(), "nothing to report: {events:?}");
    assert!(held(&v) < 1e-15);
...
fn diluting_the_beaker_releases_some_of_the_dye() {
    ...
    assert!(
        held(&v) < bound_before,
        "eleven times the water holds less on the carbon: {} against {bound_before}",
        held(&v)
    );
    assert!(
        (held(&v) + dissolved(&v) - 0.001).abs() < 1e-12,
