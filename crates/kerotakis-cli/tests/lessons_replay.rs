...
fn prepared_kids_mechanism_lessons_replay_the_computed_events() {
    let cases: &[(&str, &[&str])] = &[
        (
            ...
            &["naked egg membrane surrogate exchanged", "mol water"],
        ),
        (
...
fn newly_guided_kids_rows_keep_their_evidence() {
    // RESOLVED (2026-09-06), recorded where the next person will meet it.
    //
    ...
    // (~0.59 g), so the conserved reading is ~55.89 g whatever the
    // temperature does. It read 55.88 before the reaction carried its own
    // heat and 55.84 once the beaker cooled to 18.2 °C — and a sealed
    // vessel of plain water held 49.2302 g across a 98 K drop, so it was
    // not the heat. It was the gas: the GAS_PHASE block stated partial
    // pressures computed at the vessel's temperature and PHREEQC turned
    // them back into moles at its own default of 25 °C, so every solve of a
    // sealed vessel away from 25 °C lost or gained gas by T/298.15. Cooled
    // 14 K, a beaker handed back 95.2% of its CO₂. One `-temperature` line
    // in the aqueous tail's input; `kerotakis-phreeqc/tests/sealed_mass.rs`
    // pins the crossing on its own. The reading below is the conserved one.
    let cases: &[(&str, &[&str])] = &[
        ...
        //   284.18 / 1480.54          and the headspace at its own temperature
        //
        // The fourth move is not the lesson changing either: the gas phase
        // had been handed to PHREEQC at 25 °C whatever the beaker read, so
        // the cooled bottle lost 2.3% of its CO₂ on every solve. With the
        // gas at 18.2 °C the pressure rises back toward the first figure,
        // as the ideal gas law says it must for a fixed amount cooled 7 K.
        //
        ...
        //
        // The pressure-controlled boundary is pinned here too, because the
        // sealed pair alone never showed which boundary a number came from.
        // What the row actually claims is checked by
