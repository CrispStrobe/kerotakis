...
/// TEMPORARY diagnostic (one CI round): what `aq-023` departs on now, key
/// by key. Fails on purpose so the numbers print.
#[test]
fn probe_aq_023_order() {
    let mut report = String::new();
    for (prompt, steps) in &selected(Rule::Order) {
        if prompt.id != "aq-023" {
            continue;
        }
        let Some(case) = pair(Rule::Order, steps) else {
            report.push_str("no pair\n");
            continue;
        };
        report.push_str(&format!("verdict: {:?}\n", verdict(Rule::Order, &case)));
        let mut rows: Vec<(f64, String)> = case
            .before
            .iter()
            .filter_map(|(key, a)| {
                let b = case.after.get(key)?;
                if indistinguishable(key, *a, *b, ORDER_TOLERANCE) {
                    return None;
                }
                ...
                Some((
                    rel,
                    format!(
                        "  {key}: {a:.10e} vs {b:.10e} ({rel:.3e} rel, {:.3e} abs)",
                        (a - b).abs()
                    ),
                ))
            })
        ...
        report.push_str(&format!("{} distinguishable keys\n", rows.len()));
        for (_, line) in rows.iter().take(25) {
            report.push_str(line);
            report.push('\n');
        }
        for (key, a) in &case.before {
            if key.contains("base_equivalents") || key.contains("water") {
                let b = case.after.get(key).copied().unwrap_or(f64::NAN);
                report.push_str(&format!("  KEY {key}: {a:.17e} vs {b:.17e}\n"));
            }
        }
    }
    panic!("{report}");
