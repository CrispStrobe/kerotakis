...
/// The same chemistry, in three voices. Never generated by a language
/// model: these are written by a person and checked against the solvers.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl Codex {
    ...
    pub fn structural_problems(&self) -> Vec<String> {
        ...
        for r in &self.reactions {
            ...
            for (name, text) in [
                ("child", &r.registers.child),
                ("student", &r.registers.student),
                ("expert", &r.registers.expert),
            ] {
                if text.trim().is_empty() {
                    problems.push(format!("{}: empty {name} register", r.id));
                }
            }
