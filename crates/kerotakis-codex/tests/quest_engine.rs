...
fn the_lint_catches_prose_naming_what_the_reaction_does_not() {
    let mut clock = shipped_iodine_clock();

    say_at_lv3(
        &mut clock,
        "The iodate–bisulfite rate law is consuming NaHSO₃.",
    );
    ...
    say_at_lv3(
        &mut clock,
        "The iodate–bisulfite rate law is consuming HSO₃⁻ — the bottle \
         dissolves to the ion, and the law is keyed on the ion.",
    );
    let problems = quest::lint(std::slice::from_ref(&clock));
...
fn a_nudge_bound_to_a_reaction_that_does_not_exist_fails() {
    let mut clock = shipped_iodine_clock();
    landolt(&mut clock).when = "reacted:landolt".into();
    let problems = quest::lint(std::slice::from_ref(&clock));
    assert!(
...
fn shipped_iodine_clock() -> quest::QuestSpec {
    quest::load_dir(std::path::Path::new("../../quests"))
        .expect("quests load")
        .into_iter()
...
fn landolt(clock: &mut quest::QuestSpec) -> &mut quest::Nudge {
    clock
        .nudges
        .iter_mut()
        .find(|nudge| nudge.when.starts_with("reacted:iodate-bisulfite"))
        .expect("the Landolt nudge")
}

fn say_at_lv3(clock: &mut quest::QuestSpec, text: &str) {
    landolt(clock).say.lv3 = text.into();
}
