...
/// `charge_before`/`charge_after` carry the free acid or base, which the
/// readback keeps as charge rather than as portions.
///
...
pub fn heat_released_j(
    ...
    // The unwritten hydroxide: a positive solute charge is that much free
    // base. Free acid is H+, a master species, and contributes nothing.
    let oh = |charge: f64| if charge > 0.0 { charge } else { 0.0 };
    let d_oh = oh(charge_after) - oh(charge_before);
