...
const DISSOLUTION_NOTES: &[(&str, &str)] = &[
    ...
    ("KOH", "-57.6 kJ/mol. THE HANDBOOK CITATION THIS ROW CARRIED IS WITHDRAWN (2026-09-13): the CRC Handbook is on PLAN.md's commercial row and may not stand behind a shipped number. What survives is the check anyone can run, and it is the row's whole support: the value is consistent with K+(aq) about -252.4 and OH-(aq) about -230.0 against KOH(s) about -424.8 kJ/mol, formation enthalpies that come from the NBS/Wagman 1982 tables, which are a United States Government work. The arithmetic is the citation now."),
    ("NH4Cl", "+14.78 kJ/mol at infinite dilution. THE HANDBOOK CITATION THIS ROW CARRIED IS WITHDRAWN (2026-09-13): the CRC Handbook is on PLAN.md's commercial row. This is the one row here quoted to two decimal places and the precision was the handbook's, so the withdrawal costs it more than the others: with the citation gone there is nothing left that justifies the second decimal, and a reviewer should either find this row a primary measurement with a DOI or cut it back to +14.8. It is left at +14.78 because changing a number is a separate decision from sourcing one, which is the rule the rest of this tranche already follows."),
    ("Na2SO4", "-2.43 kJ/mol for the anhydrous salt at infinite dilution. THE HANDBOOK CITATION THIS ROW CARRIED IS WITHDRAWN (2026-09-13): the CRC Handbook is on PLAN.md's commercial row. The vendored Thenardite delta_h was examined again as the replacement and still is not one: it is -2.39 kJ in wateq4f.dat and -9.12 kJ in minteq.v4.dat, and two vendored files that disagree by a factor of four cannot between them support a number to two decimal places. wateq4f.dat's -2.39 is close enough to be tempting, which is the reason to say plainly that picking the closer of two disagreeing files is choosing an answer, not sourcing one. This row now rests on nothing."),
...
const FERMENTATION_RATE_CITATION: &str = "Kerotakis fermentation-rate calibration v1: the first-order rate constants of the four declared cultures in kerotakis/material-recipes-v1, and what they rest on. THE CONSTANTS ARE FITTED, NOT MEASURED, and that is the first thing this row says because it is the thing a reader will otherwise get wrong. MaterialRole::FermentationCulture holds a rate PER GRAM OF CULTURE PER LITRE OF LIQUID, and no measurement of that quantity exists for any of these materials: not one of them names a strain, a product form or a cell count for a specific activity to belong to, and the yoghurt recipe says so in its own words - 'NO SPECIES AND NO STRAIN IS NAMED'. What the literature measures is a fermentation - a dose, a temperature, a time and an extent - and each constant below is what makes this engine's rate law reproduce one published measurement of one. The dose it is quoted against is DECLARED, in exactly the sense kerotakis_core::fermentation::REFERENCE_VOLUME_LITRES is declared, and the timescale is cited. Confusing the two would turn a fitted constant into a claimed activity. THE RATE LAW, written out so the arithmetic below can be checked: a culture contributes k = rate * active * (1 L) / litres, where `active` is the weighed grams times a hydration factor times exp(-((T - optimum)/width)^2) and `litres` is the vessel's liquid volume; the extent over a step is 1 - exp(-k*t) and is spent as a fraction of the substrate present. THE LACTIC ANCHOR is two open-access papers, both read on 2026-09-16 through the Europe PMC REST full-text service (https://www.ebi.ac.uk/europepmc/webservices/rest/PMCxxxxxxx/fullTextXML), because neither publisher's own site would answer an automated request. THE TIME comes from M. Kim, S. Oh and J.-Y. Imm, 'Buffering Capacity of Dairy Powders and Their Effect on Yoghurt Quality', Korean Journal for Food Science of Animal Resources 38(2) (2018) 273-281, doi:10.5851/kosfa.2018.38.2.273, PMC5960825, CC BY-NC: milk carrying a commercial starter at 0.06% was held in a 42 degC water bath, and the control yoghurt reached pH 4.5 in EIGHT HOURS - the paper's own sentence is that for the high-buffering treatment 'the time to reach pH 4.5 was increased from 8 to 12 h'. THE EXTENT comes from A. Jankowska, M. Wachowska, A. Dabrowska, M. Bielecka, J. Warechowski and A. Potas, 'Evaluation of the Fermentation Profiles and Quality Attributes of Yogurts Made from Cow, Goat, and Mixed Milk', Foods 15(2) (2026) 314, doi:10.3390/foods15020314, PMC12841329, CC BY 4.0: milk was fermented at 43 degC to pH 4.6, and Table 1 gives cow milk 6.06% lactose against 5.69% in the yogurt made from it, so 6.106% of the lactose was converted. THE LACTIC ARITHMETIC. 43 degC is the yoghurt recipe's declared optimum, where the temperature envelope is exactly 1; Kim's 42 degC sits 0.4% below it on that envelope, which is well inside the rounding of everything else here, so the two papers are treated as one point. The declared dose is the one every bench script pours - one gram of culture in 100 mL of the whole-milk recipe, whose liquid volume is 100 mL x 1.03 g/mL x 0.87 water = 89.61 g = 0.08961 L. Then k = -ln(1 - 0.06106) / 28800 s = 2.1875e-6 per second, and rate = k x 0.08961 L / 1 g = 1.96e-7 per second per gram per litre. FED BACK THROUGH THE ENGINE IT LANDS 0.2% LOW, and the two halves of that are named rather than absorbed: the constant is quoted to three figures, which is already more than the cited inputs support, and the 0.08961 L above is the WATER 100 mL of milk carries, by mass, where the bench's own liquid volume is a shade larger because the serum ions occupy space too. Neither is worth a fourth figure on a number fitted from two three-figure measurements, and `crates/kerotakis-core/tests/fermentation.rs` pins the pair to half a per cent with that reasoning written beside the tolerance. It replaces 0.00001, which was an editorial classroom timescale with nothing behind it, and it is 51 times smaller. THE ALCOHOLIC ANCHOR is one open-access paper read the same day and the same way: J. Pagliardini, G. Hubmann, S. Alfenore, E. Nevoigt, C. Bideaux and S. E. Guillouet, 'The metabolic costs of improving ethanol yield by reducing glycerol formation capacity under anaerobic conditions in Saccharomyces cerevisiae', Microbial Cell Factories 12 (2013) 29, doi:10.1186/1475-2859-12-29, PMC3639890, CC BY 2.0. Its sentence 'The maximum specific ethanol production rates were reduced by 57% and 85% in TEFmut7 and TEFmut2, respectively compared to the wild type (1.5 g ethanol g DCW-1 h-1)' gives the wild-type CEN.PK113-7D figure, measured in anaerobic fed-batch culture at 30 degC. THE ALCOHOLIC ARITHMETIC. One gram of dry yeast is taken as one gram of dry cell weight, and the declared substrate is the classroom lesson's own, 100 g/L of sucrose, in one litre at 30 degC. 1.5 g of ethanol is 0.032561 mol, which on the balanced route C12H22O11 + H2O -> 4 C2H5OH + 4 CO2 is 0.0081402 mol of sucrose out of 0.29215 mol present, an extent of 0.027864 in one hour; k = -ln(1 - 0.027864)/3600 = 7.8497e-6 per second, the 30 degC envelope for a 35 degC optimum of width 18 K is 0.92574, and rate = 7.8497e-6 / 0.92574 = 8.48e-6 per second per gram per litre. It replaces 0.0002 and is 23.6 times smaller. WHAT THE FITS DO NOT BUY, stated once for all four constants. (1) ONE POINT, NOT A CURVE. This rate law is first order in the culture and first order in the substrate and the real ones are neither: lactic cultures and yeasts GROW, so a real rate rises with time, and their sugar uptake is saturated, so a real rate does not fall as the sugar does. Each constant makes the model pass through one published point; the shape of the curve through it is not claimed, and an extent read at any other time is an extrapolation. (2) NO STOP. There is no inhibition by the acid or the alcohol the culture itself makes, so the modelled extent goes to one. Real yoghurt halts near pH 4.4-4.6 because the organisms poison themselves; wait long enough on this bench and all of the lactose is converted at a pH no yoghurt has ever had. (3) LINEAR IN THE DOSE. Kim's inoculum was 0.06% of a named commercial freeze-dried powder - seventeen times lighter in grams than the gram this bench spoons into 100 mL, and by the usual cell counts some tens of times richer in organisms than that gram would be as live yoghurt. The fit reconciles them by declaring the dose rather than converting it, so an engine answer is linear in the dose where reality is closer to logarithmic in it. (4) THE ALCOHOLIC CONSTANT IS AN UPPER BOUND TWICE OVER: 1.5 g/gDCW/h is a maximum for a laboratory strain growing in mineral medium in a fermenter rather than for rehydrated supermarket yeast in cold sugar water, and one gram of active dry yeast is not one gram of viable cells. Both approximations push the same way. (5) pH WAS DELIBERATELY NOT FITTED, and fitting it would have been the error this whole tranche exists to avoid. A computed fermented-milk pH carries a second, independent defect: whole_milk resolves the serum minerals and, since 2026-09-18, the colloidal calcium phosphate, but not casein - and the caseins are about 35% of milk's buffer capacity on the ranking that follows: soluble minerals 40%, caseins 35%, CCP 20%, whey proteins 5%. THAT SPLIT IS CITED AT ONE REMOVE AND THE REMOVE IS STATED: it is F. Salaun, B. Mietton and F. Gaucheron, 'Buffering capacity of dairy products', International Dairy Journal 15(2) (2005) 95-109, doi:10.1016/j.idairyj.2004.06.007, AND THAT PAPER WAS NOT READ - sciencedirect.com answers an automated request for it with HTTP 403. What was read is Kim et al. 2018 quoting it and naming it as the source, in the sentence 'The relative contribution of milk constituents for the BC is ranked as follows: soluble minerals (40%), caseins (35%), CCP (20%), and whey proteins (5%)'. No shipped number is derived from it; it carries a BOUNDARY. That boundary was 'a computed pH is a LOWER bound at any acid dose' while neither the caseins nor the CCP were here; since the CCP was booked on 2026-09-18 it is narrower and stranger - above pH 5 the recipe now has the buffer that matters, with the amount of colloid uncertain by 40% which is 0.47 of a pH unit at a yoghurt's acidity, and below pH 5 the colloid is spent and it is a lower bound again and a worse one, because nothing is left to resist. Kim et al. also report that casein micelles peak in buffer capacity at pH 5.2, which is inside the interval a fermentation crosses, so the missing constituents carry more than their global share exactly where it matters. WHAT THAT PERCENTAGE IS NOT IS A CORRECTION FACTOR: crates/kerotakis-phreeqc/tests/lactate_speciation.rs measured what the absence actually cost - 0.66 of a pH unit at a real yoghurt's acidity, which is less than a flat reading of the 60% predicts - and docs/milk-buffer-and-the-fermentation-rate.md says why the two are not in conflict and what the 2026-09-18 half of the repair changed. A rate fitted to make pH 4.5 appear would have cancelled a rate error against a buffer error and left both in place. The rate is fitted against an extent, which is a rate; the pH is reported as an output. WHAT IS STILL EDITORIAL. The acetic and heterolactic constants are the lactic fit CARRIED ACROSS, not fitted: no measurement of an acetification or a sourdough rate per gram of culture was found, and each role's rate_evidence says Editorial rather than Derived so that the difference is data and not prose. The three optimum temperatures and the three envelope widths are unchanged and remain editorial. Compiled 2026-09-16";
...
/// `(species key, record id prefix, which latent heat, J/mol)`.
///
...
pub fn export_current_registry() -> Result<RegistryDocument, String> {
    ...
    const REVIEWED_PARAMETER_SOURCES: &[&str] = &[
        ...
        "literature/melcher-barium-sulphate-1910",
...
fn export_material_recipes(document: &mut RegistryDocument) {
    ...
    // The rates the four cultures run at are no longer this source's. They
    // were editorial classroom timescales until 2026-09-16, they are now
    // fitted to published fermentations, and a fitted constant and a
    // teaching surrogate are different claims that must not share one
    // citation - the recipes stay `confidence = surrogate` whatever their
    // rate rests on, so the recipe-level evidence cannot carry the
    // distinction. `MaterialRole::FermentationCulture::rate_evidence`
    // points here instead.
    document.sources.push(SourceRecord {
        id: FERMENTATION_RATE_SOURCE.to_string(),
        citation: FERMENTATION_RATE_CITATION.to_string(),
        licence: "AGPL-3.0-or-later".to_string(),
        lane: SourceLane::Runtime,
        origin: Some("crates/kerotakis-registry-export/src/lib.rs".to_string()),
        revision: Some("1".to_string()),
        retrieved: Some("2026-09-16".to_string()),
    });
    ...
    // A rate this project FITTED to a cited fermentation. `Derived` and not
    // `Measured`: the cited source measured a fermentation, and nobody
    // measured an activity per gram of any of these materials.
    let fitted_rate = |detail: &str| Evidence {
        source_id: FERMENTATION_RATE_SOURCE.to_string(),
        method: Method::Derived(detail.to_string()),
    };
    // A rate carried across from the lactic fit with no measurement of its
    // own. `Editorial` is the honest kind and the reason this field exists:
    // two of the four cultures now rest on a citation and two do not, and a
    // reader who cannot see which is which will take all four for the same
    // sort of number.
    let inherited_rate = |detail: &str| Evidence {
        source_id: FERMENTATION_RATE_SOURCE.to_string(),
    ...
    document.material_recipes.extend([
        ...
        MaterialRecipe {
            ...
            basis: MaterialBasis::MassFraction,
            bulk_density: Some(density(1.03)),
            // The diffusible (serum) phase of milk, and only that. Every
            // fraction is grams per gram of milk; the mmol per 100 g each
            // one works out to is in the lot assumptions, beside where the
            // number came from. Water stays first because the expansion
            // order is the recipe order and two tests read it by index.
            components: vec![
                ...
                component("octacalcium_phosphate", 0.0018345),
            ],
            unresolved_fraction: Some(FractionRange {
                lower: 0.122_014_9,
                upper: 0.122_014_9,
            ...
            preparation: Some(
                "generic unflavoured whole-cow's-milk teaching surrogate: 87% water plus the diffusible mineral buffer of milk serum - potassium, sodium, chloride, the soluble share of the calcium, inorganic phosphate and citrate - with casein, milk fat, lactose and the colloidal calcium phosphate conserved together as unresolved milk solids"
                    .to_string(),
            ),
            lot_assumptions: vec![
                ...
                "THE COLLOIDAL CALCIUM PHOSPHATE IS BOOKED, AND IT IS BOOKED FROM THE PHOSPHORUS RATHER THAN FROM THE CALCIUM. 183.45 mg per 100 g of octacalcium phosphate, Ca4H(PO4)3:3H2O - 0.3667 mmol of the database's half formula unit, carrying 1.100 mmol of phosphorus (34.1 mg, the same mass as the diffusible inorganic share above) and 1.467 mmol of calcium (58.8 mg). It enters as a SOLID, so it changes nothing about fresh milk and that is not a coincidence: the serum is already at this phase's saturation, so adding more of the solid moves no dissolved amount at all. Measured, the pH is 6.5636 with it and 6.5636 without, the ionic strength 0.07412 either way, and the serum the solver hands back is unchanged at Ca 8.1 and inorganic phosphate 10.1 mmol per kg of water. What it changes is what happens when acid arrives, which is the whole point: the solid dissolves and takes five protons per formula unit with it, which is what milk's colloidal calcium phosphate really does between pH 6.6 and about 4.9. WHY THE PHOSPHORUS AND NOT THE CALCIUM, because the two budgets do not agree and the disagreement is the size of the defect being repaired. FDC 746782 gives 101 mg of total P per 100 g; the 34.1 mg booked as diffusible inorganic leaves 66.9 mg, which is colloidal inorganic phosphate plus what is esterified onto casein and onto the phospholipids. The assumption above already calls the diffusible share 'roughly one third' of the total, and the colloidal inorganic third is the same size, so 34.1 mg of it is booked here and the last third is left where it is. Booking the MINERAL from the colloidal CALCIUM instead - the 83 mg that FDC's 123 mg leaves once the 40 mg diffusible share is taken out, which is about 280 mg of octacalcium phosphate - assumes all of milk's colloidal calcium is in the mineral, and it is not: part of it is bound directly to casein's phosphoserine and carboxyl groups and is not a phosphate phase at all. THE TWO READINGS ARE 40% APART AND LAND 0.47 OF A pH UNIT APART at a yoghurt's acidity - 4.60 on the phosphorus budget against 5.07 on the calcium one - which is the same size as the error this addition repairs. Neither is a citation for the quantity that actually decides it, which is how much phosphate the colloid releases between pH 6.6 and 4.6; that is a titration curve, it is the same one casein's half is waiting on, and Salaun 2005 is behind a 403. WHAT IS LEFT UNBOOKED, so the budgets can be checked: 24.2 mg of calcium (0.60 mmol, 19.7% of the total) that real milk carries bound directly to casein, and 32.9 mg of phosphorus (1.06 mmol) that is esterified onto casein or carried by the phospholipids. Neither is a solute and neither is modelled".to_string(),
                "WHAT IS IN THE GLASS AS A SOLID, AND WHAT STILL IS NOT. 100 mL of this recipe holds 226.6 mg of octacalcium phosphate: the 189 mg the recipe now books as colloidal calcium phosphate, plus about 37 mg the solver lays down itself out of the serum it was handed. That is the right phase - octacalcium phosphate is what a near-neutral calcium phosphate solution actually gives at bench temperature, and it is the family milk's own colloidal calcium phosphate and the first mineral of bone belong to. IT IS STILL NOT A MODEL OF THE MICELLE. Real milk carries roughly two thirds to three quarters of its calcium in the casein micelle, of the order of 2.2 mmol or 90 mg per 100 g; this holds 1.75 mmol of calcium as the mineral, and the calcium that is bound directly to casein rather than to a phosphate is absent, because casein is absent. What was true before this and is still true is the reason the rest cannot simply be added: booking more free calcium with no polyanion to balance it drives this ion set past pH 9 before any solid forms. Booking it as a SOLID is what made this addition possible at all - a charge-neutral mineral disturbs nothing until acid comes for it. HYDROXYLAPATITE IS THE STABLE PHASE AND IS DELIBERATELY WITHHELD, by a kinetic floor on its own registry row, and the aqueous tail says so out loud on every vessel of this milk: fresh milk serum really is about eight log units supersaturated against apatite and really does not precipitate it, because the micelle holds its calcium phosphate as stabilised nanoclusters. Letting it precipitate here would take the milk to pH 5.7 and strip 58% of the calcium out of the serum, which is a beaker with no milk protein in it rather than a beaker of milk".to_string(),
                "CASEIN IS STILL NOT MODELLED AND ITS BUFFERING IS THEREFORE STILL ABSENT, which is now the largest single thing this recipe does not claim, and the one a reader is most likely to stop checking. In real milk the casein micelle is a polyanion carrying of the order of 10 mmol/kg of negative charge at pH 6.7 and holding a matching share of the serum's cations; Kim et al. 2018, quoting Salaun 2005, rank the caseins at about 35% of milk's buffer capacity, second only to the soluble minerals. None of that is here. Had every diffusible cation milk actually carries been booked as a free ion with no polyanion to balance it, this recipe would have sat near pH 7.4 rather than 6.7. What is booked instead is the ion set that balances, plus the colloidal calcium phosphate as a solid. THE ADDITION OF THAT COLLOID DOES NOT MAKE THIS RECIPE'S ACIDIFIED pH TRUSTWORTHY, and the number it now reads is exactly why the warning has to be louder rather than quieter: with the acid a cited yoghurt carries, this beaker reads pH 4.60 where that yoghurt measured 4.6, and it read 3.94 before. That agreement is not a validated buffer. The modelled colloid is exhausted at very nearly that acidity - the dose curve reads 5.21 at three quarters of the acid with 4% of the solid left, then 4.60 with none - so past it this beaker has nothing left at all, where a real one still has casein's carboxyl groups titrating. Above pH 5 the buffer that matters is the one that is here; below it, the buffer that matters is the one that is not".to_string(),
                "CONSEQUENCE FOR AN ACIDIFICATION, stated because it is what this buffer will be used for. The serum phosphate and citrate are real and carry real buffer capacity, and having them is the whole difference between milk that models as water and milk that has a pH at all; the colloidal calcium phosphate added on 2026-09-18 is the second half of that and carries most of what moves between pH 6.6 and 5. Casein's own share is still missing. A computed acidified pH from this recipe was a LOWER BOUND while only the serum was here; it is no longer safely one in either direction, because how much colloid to book is uncertain by 40% and that is 0.47 of a unit at a yoghurt's acidity. Above pH 5 it should be read as a real buffer with a stated uncertainty; below pH 5, where the colloid is spent and casein's carboxyl groups would be doing the work, it is a lower bound again and a worse one than before, because nothing is left to resist. The aqueous tail says so on the wire, on every vessel of this milk it acidifies".to_string(),
        ...
        MaterialRecipe {
            ...
            roles: vec![MaterialRole::FermentationCulture {
                reference_rate_per_second_per_gram_per_litre: 0.000_008_48,
                optimum_temperature_k: 308.15,
                temperature_width_k: 18.0,
                requires_hydration: true,
                metabolism: CultureMetabolism::Alcoholic,
                rate_evidence: fitted_rate(
                    "fitted 2026-09-16 so that one gram of this yeast in one litre of 100 g/L sucrose at 30 degC gives up 1.5 g of ethanol in the first hour - the wild-type maximum specific ethanol production rate of Pagliardini et al. 2013, Microb Cell Fact 12:29, doi:10.1186/1475-2859-12-29. An UPPER BOUND and not a best estimate: that rate belongs to a laboratory strain growing in mineral medium in a fermenter, and a gram of active dry yeast is not a gram of viable cells. One point; the shape of the curve through it is not claimed",
                ),
            }],
            preparation: Some(
            ...
            lot_assumptions: vec![
                ...
                "THE SUCROSE-FERMENTATION RATE IS FITTED AS OF 2026-09-16 AND IS NO LONGER AN EDITORIAL CLASSROOM TIMESCALE. 0.00000848 PER SECOND PER GRAM OF YEAST PER LITRE of liquid at the declared 35 degC optimum is what makes this engine give up 1.5 g of ethanol in the first hour from one gram of yeast in one litre of 100 g/L sucrose at 30 degC, which is the wild-type maximum specific ethanol production rate of Pagliardini et al. 2013 (Microbial Cell Factories 12:29, doi:10.1186/1475-2859-12-29, read open-access on 2026-09-16). It replaces 0.0002 and is 23.6 times smaller. The visible difference: the classroom fair-test lesson used to take 58.6% of its sucrose in ten minutes, which is 1.7 litres of carbon dioxide out of a 100 mL beaker, and now takes 3.7%, which is about 105 mL and is a balloon. IT IS AN UPPER BOUND AND NOT A BEST ESTIMATE, twice over: the cited rate is a maximum for a laboratory strain growing in mineral medium in a fed-batch fermenter rather than for rehydrated supermarket yeast in cold sugar water, and one gram of active dry yeast is taken as one gram of dry cell weight when some of that gram is neither cell nor viable. Both approximations push the same way, so this bench still ferments faster than a kitchen does. WHAT THE FIT DOES NOT BUY, and none of it is new: this rate law is first order in the culture and first order in the substrate, and the real ones are neither - the organisms GROW, so a real rate rises with time, and their sugar uptake is saturated, so a real rate does not fall as the sugar does. There is no lag phase, no inhibition by what the culture itself makes and therefore NO STOP, so a long enough wait converts every last molecule of substrate. One published point is matched; the curve through it is not claimed. See the kerotakis/fermentation-rate-calibration-v1 source for the arithmetic and for the five things it does not buy. Strain growth, oxygen switching, inhibition and secondary metabolites remain unresolved, AND SO DOES THE HYDRATION RAMP BESIDE THIS LINE, whose time constant is six seconds at room temperature where a real dry yeast wants ten to fifteen minutes - that is a separate unfitted number and it is recorded here rather than folded into the rate".to_string(),
            ],
            substitutions: Vec::new(),
            confidence: MaterialConfidence::Surrogate,
            expansion_policy: MaterialExpansionPolicy::Fixed,
        ...
        MaterialRecipe {
            ...
            roles: vec![MaterialRole::FermentationCulture {
                ...
                metabolism: CultureMetabolism::Alcoholic,
                rate_evidence: fitted_rate(
                    "the dry-yeast fit of Pagliardini et al. 2013 (Microb Cell Fact 12:29, doi:10.1186/1475-2859-12-29). The constant is shared per gram of CONSERVED SOLIDS, which the engine enforces on its own: this recipe resolves its 70% water into the vessel, so a gram of it presents 0.3 g to the rate where a gram of dry yeast presents 1.0",
                ),
            }],
            preparation: Some(
            ...
            lot_assumptions: vec![
                ...
                "THE SUCROSE-FERMENTATION RATE IS FITTED AS OF 2026-09-16 AND IS NO LONGER AN EDITORIAL CLASSROOM TIMESCALE. It is the dry-yeast constant, 0.00000848 PER SECOND PER GRAM PER LITRE at the declared 35 degC optimum, fitted to the wild-type maximum specific ethanol production rate of Pagliardini et al. 2013 (Microbial Cell Factories 12:29, doi:10.1186/1475-2859-12-29) and 23.6 times smaller than the 0.0002 it replaces. IT IS SHARED PER GRAM OF DRY SOLIDS AND THE SHARING IS STRUCTURAL RATHER THAN STATED. The engine charges the rate with the grams of this material that stay UNRESOLVED, and this recipe's water is resolved into the vessel's own water, so a gram of it presents 0.3 g to the rate where a gram of dry yeast presents very nearly 1.0 - the same scaling by dry solids the catalase composition carries explicitly. The two halves of this recipe therefore agree about what a gram of it is, which is worth writing down because nothing in the role says so and the mechanism is easy to read the wrong way round. WHAT THE FIT DOES NOT BUY, and none of it is new: this rate law is first order in the culture and first order in the substrate, and the real ones are neither - the organisms GROW, so a real rate rises with time, and their sugar uptake is saturated, so a real rate does not fall as the sugar does. There is no lag phase, no inhibition by what the culture itself makes and therefore NO STOP, so a long enough wait converts every last molecule of substrate. One published point is matched; the curve through it is not claimed. See the kerotakis/fermentation-rate-calibration-v1 source for the arithmetic and for the five things it does not buy. Strain growth, oxygen switching, inhibition and secondary metabolites remain unresolved".to_string(),
            ],
            substitutions: Vec::new(),
            confidence: MaterialConfidence::Surrogate,
            expansion_policy: MaterialExpansionPolicy::Fixed,
        ...
        MaterialRecipe {
            ...
            lot_assumptions: vec![
                "the silica is reported as the installed SiO2 species so the object's silicon and oxygen are real inventory; sharing one species key with quartz sand carries no claim about polymorph, grain structure or optical quality".to_string(),
                "the boron oxide that makes this glass take thermal shock is NOT resolved, and the reason given here was false until 2026-09-07. It read \"no shipped database defines a borate that would let it dissolve or react\", and every dataset this lab routes defines the element outright: wateq4f.dat line 16 and minteq.v4.dat line 17 both carry `B  H3BO3`, pitzer spells the same element `B  B(OH)3`, and the first two carry `H3BO3 = H2BO3- + H+` at pKa 9.24 as well. The bench speciates that couple now. The real reason the boria stays in the conserved 19% is the assumption two rows down and has nothing to do with the thermodynamics: a resolved component is one that goes into solution, and glass does not. Resolving it would not have put an invented oxide in the beaker - it would have put a real one there and then let the beaker dissolve into whatever it was holding. The bench therefore holds the composition of borosilicate without holding the property borosilicate is bought for, which is still true and was always the honest sentence".to_string(),
        ...
        MaterialRecipe {
            ...
            lot_assumptions: vec![
                ...
                "the dissolved borax is not handed to the aqueous engine, and the reason written here before was wrong about why. It said \"no shipped database is asked for it\", which was a true sentence about this lab's wiring that read as a claim about the databases: all three routed datasets define boron, two of them carry `H3BO3 = H2BO3- + H+` at pKa 9.24, and the bench speciates boric acid and borate from those rows today. What borax has that a dissolved borate does not is a curated solubility bound - 2.5 g/100 mL at 20 degC, 27.4 at 100 - and an aqueous role is phase-blind: handing this salt one would enter the crystals sitting on the bottom of the beaker into solution along with the part that actually dissolved. The bound could move to the engine instead if any routed dataset spelled a sodium borate SOLID, and none does (minteq.v4 and wateq4f between them have Pb, Zn, Cd and Co metaborates and nothing else). So the bound stays curated, the salt keeps its own key, and it still contributes no pH and no ionic strength - which the bench still says out loud".to_string(),
        ...
        MaterialRecipe {
            ...
            physical_form: MaterialPhysicalForm::Granules,
            roles: vec![MaterialRole::FermentationCulture {
                reference_rate_per_second_per_gram_per_litre: 0.000_000_196,
                optimum_temperature_k: 316.15,
                temperature_width_k: 15.0,
                requires_hydration: false,
                metabolism: CultureMetabolism::Homolactic,
                rate_evidence: fitted_rate(
                    "fitted 2026-09-16 so that one gram of this culture in 100 mL of the whole-milk recipe converts 6.106% of the milk's lactose in eight hours at the declared 43 degC optimum. The EIGHT HOURS are Kim, Oh and Imm 2018 (Korean J Food Sci Anim Resour 38:273-281, doi:10.5851/kosfa.2018.38.2.273), whose control yoghurt reached pH 4.5 in a 42 degC bath in that time; the 6.106% is Jankowska et al. 2026 (Foods 15(2):314, doi:10.3390/foods15020314) Table 1, cow milk 6.06% lactose against 5.69% in the yogurt made from it at 43 degC. THE DOSE IS DECLARED AND THE TIMESCALE IS CITED: nothing measures an activity per gram of a culture that names no strain",
                ),
            }],
            ...
            lot_assumptions: vec![
                ...
                "THE RATE IS FITTED AS OF 2026-09-16 AND THE MAGNITUDE IS NO LONGER UNESTABLISHED. 0.000000196 PER SECOND PER GRAM OF CULTURE PER LITRE OF LIQUID at the declared 43 degC optimum is what makes this engine convert 6.106% of a milk's lactose in eight hours at that optimum, for the dose every bench script pours: one gram of culture in 100 mL of the whole-milk recipe, which is 0.08961 L of liquid. THE EIGHT HOURS ARE MEASURED - Kim, Oh and Imm 2018, Korean Journal for Food Science of Animal Resources 38:273-281, doi:10.5851/kosfa.2018.38.2.273, whose control yoghurt reached pH 4.5 in a 42 degC water bath in that time. THE 6.106% IS MEASURED - Jankowska et al. 2026, Foods 15(2):314, doi:10.3390/foods15020314, Table 1, cow milk 6.06% lactose against 5.69% in the yogurt made from it at 43 degC to pH 4.6. Both were read open-access on 2026-09-16. The value is 51 times smaller than the 0.00001 that #618 carried across unchanged, and the eight-hour counter-top script now converts 1.48% of the lactose rather than 53.2%. THE DOSE IS DECLARED AND ONLY THE TIMESCALE IS CITED, and running the two together is the mistake this line exists to prevent. Nothing measures a rate per gram of 'yoghurt culture', because no strain, product form or cell count is named for one to belong to. Kim's inoculum was 0.06% of a named commercial freeze-dried powder: seventeen times lighter in grams than the gram this bench spoons into 100 mL, and by the usual cell counts some tens of times richer in organisms than that gram would be as live yoghurt. The fit reconciles them by DECLARING the dose, exactly as REFERENCE_VOLUME_LITRES declares a volume, so an engine answer is linear in the dose where reality is closer to logarithmic in it. WHAT WAS DELIBERATELY NOT FITTED IS THE pH. A computed fermented-milk pH carries a second and independent defect - the whole-milk recipe resolves the serum minerals but not casein or colloidal calcium phosphate, which are about 60% of milk's buffer capacity - so it is a LOWER bound at any acid dose. A rate chosen to make pH 4.5 appear would have cancelled a rate error against a buffer error and left both in place. The rate is fitted against an extent, which is itself a rate; the pH is reported as an output and nothing here targets it. WHAT THE FIT DOES NOT BUY, and none of it is new: this rate law is first order in the culture and first order in the substrate, and the real ones are neither - the organisms GROW, so a real rate rises with time, and their sugar uptake is saturated, so a real rate does not fall as the sugar does. There is no lag phase, no inhibition by what the culture itself makes and therefore NO STOP, so a long enough wait converts every last molecule of substrate. One published point is matched; the curve through it is not claimed. See the kerotakis/fermentation-rate-calibration-v1 source for the arithmetic and for the five things it does not buy. Cell growth, a lag phase, nutrient depletion and strain variation are all still absent, and so is the acid inhibition that stops a real yoghurt near pH 4.4-4.6".to_string(),
        ...
        MaterialRecipe {
            ...
            roles: vec![MaterialRole::FermentationCulture {
                ...
                rate_evidence: inherited_rate(
                    "INHERITED from the lactic fit of 2026-09-16, not fitted. No measurement of an acetification rate per gram of vinegar mother was found, so this route keeps the shared timescale it always had and gains no citation of its own; the method is Editorial rather than Derived so that a consumer can tell the two apart without reading prose",
                ),
            }],
            ...
            lot_assumptions: vec![
                ...
                "THE RATE IS INHERITED AND NOT MEASURED, and the role's rate_evidence says so in data rather than only here. 0.000000196 PER SECOND PER GRAM OF CULTURE PER LITRE OF LIQUID at the declared 30 degC optimum is the constant fitted to the LACTIC route on 2026-09-16, carried across to this one because no measurement of an acetification rate per gram of vinegar mother was found. The three slow cultures shared one editorial timescale before that day and share one anchored timescale now; what differs between them is still the temperature envelope and the dose, not a measured activity. What the fifty-fold recalibration does to this bench: a covered jar of 0.1 mol of ethanol in 100 mL oxidises about an eighth of its alcohol in 24 hours and has most of its oxygen left, where the carried-over constant consumed very nearly every mole of oxygen in the vessel and hid the culture behind the air supply. WHAT THE FIT DOES NOT BUY, and none of it is new: this rate law is first order in the culture and first order in the substrate, and the real ones are neither - the organisms GROW, so a real rate rises with time, and their sugar uptake is saturated, so a real rate does not fall as the sugar does. There is no lag phase, no inhibition by what the culture itself makes and therefore NO STOP, so a long enough wait converts every last molecule of substrate. One published point is matched; the curve through it is not claimed. See the kerotakis/fermentation-rate-calibration-v1 source for the arithmetic and for the five things it does not buy. Cell growth, a lag phase, inhibition by the acid the culture itself makes, nutrient depletion and strain variation are all absent".to_string(),
        ...
        MaterialRecipe {
            ...
            roles: vec![MaterialRole::FermentationCulture {
                ...
                rate_evidence: inherited_rate(
                    "INHERITED from the lactic fit of 2026-09-16, not fitted. No measurement of a sourdough rate per gram of starter was found. This bench is substrate-limited either way - the starter's 2% free sugar is gone inside eight hours at the old constant and at the new one - so the sourdough row barely moving under a fifty-fold recalibration is NOT evidence that either value is right",
                ),
            }],
            ...
            lot_assumptions: vec![
                ...
                "THE RATE IS INHERITED AND NOT MEASURED, and the role's rate_evidence says so in data rather than only here. 0.000000196 PER SECOND PER GRAM OF CULTURE PER LITRE OF LIQUID at the declared 27 degC optimum is the constant fitted to the LACTIC route on 2026-09-16, carried across to this one because no measurement of a sourdough rate per gram of starter was found. The three slow cultures shared one editorial timescale before that day and share one anchored timescale now; what differs between them is still the temperature envelope and the dose, not a measured activity. THIS ROW BARELY MOVES UNDER THE RECALIBRATION AND THAT IS NOT EVIDENCE FOR IT: a 50 g starter in 75 mL is a very concentrated culture, so its 2% free sugar was entirely gone inside eight hours at the old constant and is 83% gone at the new one, and the acid it makes falls by a sixth where the yoghurt row's falls by a factor of thirty-six. The sourdough bench is nearly substrate-limited either way and therefore cannot see a fifty-fold change in the rate. A row that does not move is a row that is not testing the number. WHAT THE FIT DOES NOT BUY, and none of it is new: this rate law is first order in the culture and first order in the substrate, and the real ones are neither - the organisms GROW, so a real rate rises with time, and their sugar uptake is saturated, so a real rate does not fall as the sugar does. There is no lag phase, no inhibition by what the culture itself makes and therefore NO STOP, so a long enough wait converts every last molecule of substrate. One published point is matched; the curve through it is not claimed. See the kerotakis/fermentation-rate-calibration-v1 source for the arithmetic and for the five things it does not buy. Cell growth, a lag phase, inhibition by the acid the culture itself makes, nutrient depletion and strain variation are all absent".to_string(),
