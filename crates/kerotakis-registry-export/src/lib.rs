    ...
    document.material_recipes.extend([
        ...
        MaterialRecipe {
            ...
            lot_assumptions: vec![
                ...
                "THE RATE IS INHERITED AND NOT MEASURED, and the role's rate_evidence says so in data rather than only here. 0.000000196 PER SECOND PER GRAM OF CULTURE PER LITRE OF LIQUID at the declared 27 degC optimum is the constant fitted to the LACTIC route on 2026-09-16, carried across to this one because no measurement of a sourdough rate per gram of starter was found. The three slow cultures shared one editorial timescale before that day and share one anchored timescale now; what differs between them is still the temperature envelope and the dose, not a measured activity. THIS ROW BARELY MOVES UNDER THE RECALIBRATION AND THAT IS NOT EVIDENCE FOR IT: a 50 g starter in 75 mL is a very concentrated culture and its 2% free sugar is gone inside eight hours at the old constant and at the new one, so the sourdough bench is substrate-limited either way and cannot see a fifty-fold change in the rate. A row that does not move is a row that is not testing the number. WHAT THE FIT DOES NOT BUY, and none of it is new: this rate law is first order in the culture and first order in the substrate, and the real ones are neither - the organisms GROW, so a real rate rises with time, and their sugar uptake is saturated, so a real rate does not fall as the sugar does. There is no lag phase, no inhibition by what the culture itself makes and therefore NO STOP, so a long enough wait converts every last molecule of substrate. One published point is matched; the curve through it is not claimed. See the kerotakis/fermentation-rate-calibration-v1 source for the arithmetic and for the five things it does not buy. Cell growth, a lag phase, inhibition by the acid the culture itself makes, nutrient depletion and strain variation are all absent".to_string(),
