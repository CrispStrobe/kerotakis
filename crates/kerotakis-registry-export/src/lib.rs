    ...
    document.material_recipes.extend([
        ...
        MaterialRecipe {
            ...
            lot_assumptions: vec![
                ...
                "THE MINERAL BUFFER IS MILK'S DIFFUSIBLE (SERUM) PHASE AND ONLY THAT, because the aqueous solver models a solution and the colloidal phase is not one. Per 100 g of milk: sodium 38.0 mg (1.65 mmol) and potassium 150.0 mg (3.84 mmol) are FDC 746782's analytical values entered whole, both being essentially fully diffusible in milk; chloride is 106.4 mg (3.00 mmol, about 30 mmol/kg), which FoodData Central does not report at all and which comes from the dairy-chemistry literature; citrate is 174.0 mg (0.92 mmol, about 9.2 mmol/kg) entered as the citrate ion C6H5O7-3, which is what nearly nine tenths of milk's citrate actually is at pH 6.7 and is also the key the aqueous tail books dissolved citrate back into; and diffusible inorganic phosphate is 1.10 mmol entered as H2PO4-, which is 34.1 mg of phosphorus out of FDC's 101 mg of total P - the roughly one third of milk's phosphorus that is both inorganic and in the serum, the rest being colloidal or esterified onto casein and therefore not a solute".to_string(),
                "THE CHLORIDE AND CITRATE FIGURES RIDE A PENDING-REVIEW PROVENANCE LANE. They are recorded as commonly tabulated in the dairy-chemistry literature for whole cow's milk - roughly 30 and 9 to 10 mmol/kg respectively - and NO positively identified copy of any single paper or edition was opened for either. General dairy-chemistry references on milk salts are the intended primary source and both rows are flagged for reviewer confirmation against one before any stronger claim is made, exactly as the phase-transition and resistivity tranches are. The sodium, potassium, calcium and phosphorus totals are not in this position: those are FDC 746782's own analytical numbers".to_string(),
                "SOLUBLE CALCIUM IS THE ONE NUMBER HERE THAT IS FITTED RATHER THAN SOURCED, and it says so rather than hiding among the sourced ones. 34.8 mg per 100 g (0.869 mmol), which is 28% of FDC's 123 mg of total calcium, is the value at which this ion set is electroneutral at fresh milk's pH of about 6.7. The dairy literature puts milk's diffusible calcium at roughly a fifth to a third of the total, so 28% lands inside the measured band - but that band is the CHECK on this number and not its source, and a reviewer should read it as a fitted parameter. The reason one number has to be fitted at all is the assumption below".to_string(),
                "CASEIN IS NOT MODELLED AND ITS BUFFERING IS THEREFORE ABSENT, which is the largest single thing this recipe does not claim. In real milk the casein micelle is a polyanion carrying of the order of 10 mmol/kg of negative charge at pH 6.7 and holding a matching share of the serum's cations, and it is also - together with the colloidal calcium phosphate bound to it - the larger part of milk's buffer capacity between pH 6.6 and pH 5.0, which is exactly the interval a yoghurt fermentation crosses. None of that is here. Had every diffusible cation milk actually carries been booked as a free ion with no polyanion to balance it, this recipe would have sat near pH 7.4 rather than 6.7. What is booked instead is the ion set that balances, and the calcium left out is the calcium that in real milk is bound to the casein as colloidal calcium phosphate - so the fit and the chemistry point the same way, which is the only thing that makes the fit defensible".to_string(),
                ...
                "magnesium, sulfate and the dissolved carbon dioxide of fresh milk are deliberately not booked. Together they are a few percent of the serum's charge and none of them buffers anywhere near pH 6.7, so leaving them in the unresolved fraction costs nothing the pH meter can see; booking them would have moved the fitted calcium out of its measured band, which is a worse trade than saying they are absent".to_string(),
